...
| 2 | [Monitoring exception bypass (LINE/CALL/INSTRUCTION)](#) | CPython 3.13 | New |
...
- **`cpython-313-monitoring-callback-crash/`** — `sys.monitoring` LINE/CALL/INSTRUCTION callback exceptions bypass `try/except` handlers (may also produce an internal error dump on some machines).
