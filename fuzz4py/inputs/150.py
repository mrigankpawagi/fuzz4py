...
def jit_sensitive_function(data):
    ...
    for i in data:
        try:
            result += i * random.randint(1, 10) # Introduce randomness
        except TypeError:
    ...
    def worker(x, res_list, lck):
      with lck:
          try:
            ...
            res_list.append(f"Error in worker: {e}") # Log exceptions
...
def test_complex_annotation(data: typing.List[typing.Union[int, str]]):
  ...
  for item in data:
    if isinstance(item, int):
      ...
      try:
        result.append(int(item) * random.randint(1, 10)) # Introduce randomness
      except ValueError:
        result.append(len(item) * random.randint(1,10)) # Introduce randomness
    else:
...
def test_replace_protocol(data):
  ...
  try:
    ...
    if hasattr(new_data, 'replace'):
        new_data.replace(0, random.randint(0, 1000))  # Fuzz with random value
        return len(new_data)
    else:
...
def worker(data, lock, db):
    ...
    with lock:
      try:
        ...
        db[key] = data
        result = int(db[key])
      except (KeyError, ValueError, TypeError, AttributeError) as e: # Expanded exception handling
        result = f"Error in worker: {e}"  # More informative error handling
    return result


def main():
  data1 = list(range(10))
  data1.extend([None, "a", 12.345]) # Add variety to data
  results1 = test_free_threading(data1)
  print("Free threading results:", results1)

  data2 = [1, 2, "3", 4, "abc", 5.5, None] + [random.randint(-100,100) for _ in range(5)] #Fuzzing with random integers
  results2 = test_complex_annotation(data2)
  print("Complex annotation results:", results2)
  
  data3 = [1, 2, 3, None, "abc"]
