...
def complex_function(input_data):
    if isinstance(input_data, list):
        try:
            ...
            return f"Error: Invalid list type ({e})"  # More informative error message
    elif isinstance(input_data, tuple):
        try:
            return str(input_data[0]) + str(input_data[1])
        except (IndexError, TypeError) as e:
            return f"Error: Invalid tuple index ({e})"  # More informative error message
    else:
