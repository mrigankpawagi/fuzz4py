    ...
    if isinstance(item, int):
      results.append(item * 2)
        ...
        try:
            ...
            #Handle non-integer strings more robustly - add a default value.
            results.append(0)
    else:
        print(f"Unexpected type: {type(item)} for {item}")
        results.append(-1)
  return results
...
def fuzz_replace_protocol(data: list):
  ...
  try:
      return copy.replace(data, value=random.randint(-100, 100)) #fuzzing new protocol with different values
...
def fuzz_ssl_connection():
  try:
    ...
    return context.check_hostname(hostname, 'test.crt')
  except Exception as e:
    return f"Error: {e}"

# Example usage - add more comprehensive test cases here
data = [1, 2, 3, 4, 5, 0]  #include a zero
complex_data = [1, "2", 3, "4a", 5, "abc", 1.23,  [1, 2]] #Added a list to test unexpected type
