...
def main():
    ...
    # dbm.sqlite3 test
    db_file = f'mydatabase_{random.randint(1,100)}.db'
    try:
        db = dbm.open(db_file, 'c')  
        db['key1'] = str(random.randint(1, 1000))  # Non-string value, but converted to string
        db.close()
        ...
        try:
            ...
            print(f"Error removing database: {e}")  # Handle potential OSError
