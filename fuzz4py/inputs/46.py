...
def my_function(arg1: int, arg2: str) -> float:
    ...
    try:
        ...
        except (OverflowError, ValueError, TypeError) as e:  # Catch more exceptions
            print(f"Error in multiplication or conversion: {e}. Returning NaN.")
            return float('nan')
    except (TypeError, ValueError, ZeroDivisionError, OverflowError) as e:
        print(f"Error in my_function: {e}, arg1: {arg1}, arg2: {arg2}, type(arg2): {type(arg2)}")  # More info
        return float('nan')
...
def worker(data: list, lock):
    for item in data:
        ...
        try:
            ...
            possible_types = [None, 123, b"bytes", [], [1, 2, 3], {1: "a"}, (1, 2), object(), 123.456, float('inf'), float('-inf'), float('nan'), True, False, 1 + 2j, bytearray(b"test")]
            for t in possible_types:
                if random.random() < 0.1:
                    arg2 = t
                    print(f"Randomly set arg2 to {arg2} type {type(arg2)}")
            try:
            ...
            print(f"  arg2: {arg2}")
            print(f"  item: {item}")
            print(f"Type of Item:{type(item)}")  # add type info
        finally:
            lock.release()


def main():
    data = [1, 2, 3, 4, 5, "a", 0, -1, 1.23, None, True, False, 100000, -100000, "test", b"bytes", [1,2], 1+2j, bytearray(b"test"), 2**1000]  # Expanded data types
    lock = threading.Lock()
    threads = []
    for item in data:
...
def test_free_threading():
    data = [1, 2, 3, 4, 5, 12, 15, 20, -5, 0, sys.maxsize, -sys.maxsize, 10000, -10000, 100000, -100000, float('inf'), float('-inf')]
    threads = []
    for _ in range(5):
...
if __name__ == "__main__":
    ...
    data_for_annotation = [(1, "a"), (2, "bb"), (3, "ccc"), (None, "extra"), (1, "invalid"), "a string", (1, 123), (0, "error"), (1, 123), (10, None), (1, "a" * 1000), (1, "invalid type"), (-1, "negative"), (0, "zero"), (1, float('inf'))]
