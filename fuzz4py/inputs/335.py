...
def main_part1():
    ...
    threads = []
    for _ in range(5):
        thread_data = copy.deepcopy(data)  # Deep copy for safety
    ...
    def my_func(arg: list[int]) -> list[int]:
...
def worker_part2(data, lock):
    try:
        with dbm.open('mydatabase', 'c') as db:  # Open in write mode within the try
