Return Code: 0
Stdout: Result of copy.replace() on list: Error: replace() does not support list objects
Result of copy.replace() on tuple: Error: replace() does not support tuple objects
Result of copy.replace() on string: Error: replace() does not support str objects
Result of dbm fuzzing: Error during dbm fuzzing: NOT NULL constraint failed: Dict.value
invalid_ca.crt not found, skipping SSL verification load
Result of SSL fuzzing: SSL fuzzing successful

Stderr: 
